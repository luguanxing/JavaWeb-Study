...
public class TestPartSearch {
	...
	@Test
	public void testIndexAddDataAndPart()throws Exception{
	...
	@Test
	public void testSearchMultiPart() throws Exception{
		SearchRequestBuilder srb = client.prepareSearch("film2").setTypes("dongzuo");
		SearchResponse response = srb.setQuery(QueryBuilders.matchQuery("title", "最后狼").analyzer(ANALYZER))
	...
	@Test
	public void testSearchMultiField() throws Exception{
		SearchRequestBuilder srb = client.prepareSearch("film2").setTypes("dongzuo");
		SearchResponse response = srb.setQuery(QueryBuilders.multiMatchQuery("非洲星球基友", "title", "content").analyzer(ANALYZER))
			.setFetchSource(new String[]{"title", "price"}, null)
			.execute()
			.actionGet();
		SearchHits hits = response.getHits();
